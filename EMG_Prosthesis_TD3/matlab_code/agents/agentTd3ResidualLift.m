function agent = agentTd3ResidualLift(observationInfo, actionInfo)
%agentTd3ResidualLift builds a residual TD3 agent on top of a frozen base.
%
% The base actor branch is copied from a checkpoint and frozen. A small
% learnable residual branch receives [obs; a_base] and outputs a bounded
% correction. This preserves the base policy exactly at initialization
...
baseCheckpointPath = string(td3Residual.baseCheckpointPath);
baseAgent = loadSavedAgent(baseCheckpointPath);
baseActor = getActor(baseAgent);
baseCritics = getCritic(baseAgent);

actor = buildResidualActor(baseActor, observationInfo, actionInfo, td3Residual, baseCheckpointPath);
...
function actor = buildResidualActor(baseActor, observationInfo, actionInfo, td3Residual, baseCheckpointPath)
baseModel = getModel(baseActor);
if ~isa(baseModel, "dlnetwork")
    error("Expected the base actor model to be a dlnetwork, got %s.", class(baseModel));
end
...
baseFc1Source = findLayerByName(baseLayers, "fc1", baseCheckpointPath);
baseFc2Source = findLayerByName(baseLayers, "fc2", baseCheckpointPath);
baseActionSource = findLayerByName(baseLayers, "action", baseCheckpointPath);
...
function layer = findLayerByName(layers, layerName, checkpointPath)
idx = find(arrayfun(@(L) string(L.Name) == string(layerName), layers), 1, "first");
if isempty(idx)
    error([ ...
        "Could not find layer '%s' in the selected base actor. " ...
        "Residual lift expects a plain TD3-style actor checkpoint. Base checkpoint: %s"], ...
        string(layerName), string(checkpointPath));
end
layer = layers(idx);
end
