...
% ETAPA 7H hold-conditioned effort term. Defaults remain inert and the
% experiment opts in through an override and an explicit reward type.
...
for fieldIdx = 1:numel(holdToleranceFields)
    fieldName = holdToleranceFields(fieldIdx);
    value = params.(fieldName);
    if ~isnumeric(value) || ~isreal(value) || ~isscalar(value) || ...
            ~isfinite(value) || value < 0
        error("configurables:InvalidIntentHoldTolerance", ...
            "%s must be a finite nonnegative scalar.", fieldName);
    end
end
