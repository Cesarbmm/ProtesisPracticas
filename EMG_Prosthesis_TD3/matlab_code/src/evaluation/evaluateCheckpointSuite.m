...
try
    ...
    ensureDirectoryExists(phaseAFolder);
    ensureDirectoryExists(phaseBFolder);
    ...
    includePhaseBPaths = resolvePhaseBIncludePaths(options, selectionInfo);
    shortlistPaths = unique([shortlistPaths(:); includePhaseBPaths(:)], "stable");
...
defaults = struct( ...
    "experimentDir", "", ...
    "checkpointPaths", string.empty(1, 0), ...
    "phaseBIncludeCheckpointPaths", string.empty(1, 0), ...
...
options.phaseBIncludeCheckpointPaths = string(options.phaseBIncludeCheckpointPaths);
...
if ~isstruct(options.phaseBIncludePolicy)
    error("options.phaseBIncludePolicy must be a struct.");
end
end
...
function includePaths = resolvePhaseBIncludePaths(options, selectionInfo)
includePaths = string.empty(0, 1);

if ~isempty(options.phaseBIncludeCheckpointPaths)
    includePaths = options.phaseBIncludeCheckpointPaths(:);
end

policy = options.phaseBIncludePolicy;
if isempty(policy) || isempty(fieldnames(policy))
    includePaths = unique(includePaths, "stable");
    return;
end

experimentDir = string(selectionInfo.experimentDir);
if strlength(experimentDir) == 0 || ~isfolder(experimentDir)
    includePaths = unique(includePaths, "stable");
    return;
end

mode = lower(string(getPolicyField(policy, "mode", "all")));
switch mode
    case {"", "none"}
        policyPaths = string.empty(0, 1);
    case "episode_window"
        windowValue = double(getPolicyField(policy, "window", []));
        if numel(windowValue) ~= 2
            error("phaseBIncludePolicy.window must be a two-element episode range.");
        end
        policyPaths = discoverCheckpointsInExperiment(experimentDir, struct("mode", "all"));
        episodes = arrayfun(@inferEpisodeFromCheckpointName, policyPaths);
        policyPaths = policyPaths(episodes >= windowValue(1) & episodes <= windowValue(2));
    otherwise
        policyPaths = discoverCheckpointsInExperiment(experimentDir, policy);
end

includePaths = unique([includePaths(:); policyPaths(:)], "stable");
end
...
row = struct( ...
    ...
    "encoder2FlexVariant", string(getMetaField(checkpointMeta.override, "encoder2FlexVariant", "")), ...
    "motor2Encoder2FlexGapOffset", double(getMetaField(checkpointMeta.override, "motor2Encoder2FlexGapOffset", NaN)), ...
    "motor2Encoder2FlexBreakOffset", double(getMetaField(checkpointMeta.override, "motor2Encoder2FlexBreakOffset", NaN)), ...
...
overrideFields = [ ...
    ...
    "rewardMotorWeights", ...
    "rewardActionMotorWeights", ...
    "rewardDeltaActionMotorWeights", ...
    ...
    "actionCommandLevels", ...
    "actionCommandLevelsByMotor", ...
    "encoder2FlexVariant", ...
    "motor2Encoder2FlexGapOffset", ...
    "motor2Encoder2FlexBreakOffset", ...
