...
if this.episodeCounter < 1 || this.c < 1
    return
end
...
for i = 1:4
    ...
    for j = 1:length(action_indices)
        ...
        if ~isnan(action_value)
            plot(ax1, action_indices(j), prosthesis_position_interp(round(action_indices(j)), i), 'r.');
            text(ax1, action_indices(j), prosthesis_position_interp(round(action_indices(j)), i), sprintf('%.2f', action_value), ...
    ...
    for j = 1:length(action_indices)
        ...
        if ~isnan(action_sat_value)
            plot(ax2, action_indices(j), prosthesis_position_interp(round(action_indices(j)), i), 'rx', 'MarkerSize', 6);
            text(ax2, action_indices(j), prosthesis_position_interp(round(action_indices(j)), i), sprintf('%.2f', action_sat_value), ...
                'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'left', 'Color', 'black', 'FontSize', 11);
        end

        reward_value = aux_reward_vectors(j, i);
    ...
    if i == 1
        title(ax1, sprintf("Episode %d", this.episodeCounter))
        title(ax2, sprintf("Episode %d", this.episodeCounter))
    elseif i == 4
...
filename = fullfile(saveDir, sprintf('episode_%d.png', this.episodeCounter));
saveas(f, filename);
fprintf('Saved %s\n', filename);
close(f);
end
