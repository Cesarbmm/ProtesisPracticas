...
switch this.observationVariant
    ...
    case {"intentMarkov60", "intentDeclaredRestHoldMarkov62"}
        ...
        if this.observationVariant == "intentDeclaredRestHoldMarkov62"
            if ~isscalar(this.intentDeclaredRest) || ...
                    ~islogical(this.intentDeclaredRest) || ...
                    ~isscalar(this.intentHoldLatch) || ...
                    ~islogical(this.intentHoldLatch) || ...
                    ~isfinite(this.intentHoldPositionMse) || ...
                    this.intentHoldPositionMse < 0
                error("Env:InvalidIntentDeclaredRestHoldState", ...
                    "Declared-rest hold state is invalid.");
            end
            state = [state; double(this.intentDeclaredRest); ...
                double(this.intentHoldLatch)];
        end
