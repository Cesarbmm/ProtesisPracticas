% Fixture congelada desde f59798acb972f95815e2499dd828c3f8725d70d7.
% Unicos cambios: nombre publico y resolucion de las dos rutas MAT al directorio fixture.
% No cambiar la aritmetica: oraculo independiente de la implementacion E0P.
function trajectory = plantE0HistoricalReference( ...
...
if isempty(sim_params)
    f_name = fullfile(fileparts(mfilename("fullpath")), "fit_C2.mat");
    f = load(f_name);
    sim_params = f.params;
    tail_length = f.tail_length;

    f_name = fullfile(fileparts(mfilename("fullpath")), "pattern_curve.mat");
