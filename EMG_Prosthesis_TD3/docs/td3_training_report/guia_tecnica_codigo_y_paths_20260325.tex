...
\texttt{src/runtime/trainInterface.m} & Entry point operativo para entrenamiento y evaluacion. \\
...
\texttt{src/reward\_functions} & Reward actual y variantes historicas o experimentales. \\
\texttt{src/@SimController} & Simulador de la protesis y capa de control de motores. \\
\texttt{src/evaluation/runCheckpointAudit.m} & Auditoria automatizada de checkpoints. \\
\texttt{src/evaluation/runCheckpointTest.m} & Test directo de un checkpoint con simulacion y plots. \\
...
\makecell[tl]{\texttt{src/runtime/}\\\texttt{trainInterface.m}} & Launcher & Orquesta carga del agente, carga del dataset, creacion de entorno y ejecucion de \texttt{train} o \texttt{sim}. \\
...
\makecell[tl]{\texttt{src/reward\_functions/}\\\texttt{trackingMseActionRateReward.m}} & Reward actual & Penaliza tracking MSE, magnitud de accion y cambio de accion. \\
...
\makecell[tl]{\texttt{src/evaluation/}\\\texttt{runCheckpointAudit.m}} & Infraestructura & Lanza auditorias reproducibles de checkpoints. \\
\makecell[tl]{\texttt{src/evaluation/}\\\texttt{runCheckpointTest.m}} & Evaluacion & Ejecuta una simulacion batch sobre un checkpoint concreto. \\
\bottomrule
\end{tabularx}
\normalsize
\end{table}
...
\makecell[tl]{\texttt{defaultCheckpoint}\\\texttt{Path()}} & \makecell[tl]{\texttt{src/evaluation/}\\\texttt{runCheckpointTest.m}} & \makecell[tl]{Solo si llamas a\\\texttt{runCheckpoint}\\\texttt{Test()}\\sin argumentos} & Ese fallback es historico; lo correcto es pasar la ruta explicitamente. \\
...
\item \texttt{.\textbackslash src}, \texttt{.\textbackslash agents}, \texttt{.\textbackslash lib}: los \texttt{addpath} ya se resuelven desde \texttt{src/runtime/trainInterface.m}.
